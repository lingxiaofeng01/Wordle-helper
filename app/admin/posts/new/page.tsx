...
import { ArrowLeft, Save, Eye, Upload, X, PlusCircle, Loader2, Image, CheckCircle, AlertCircle, Clock, Star, Plus } from 'lucide-react'
...
// 自动保存的键名
const AUTO_SAVE_KEY = 'blog_post_draft'
...
export default function NewPostPage() {
  ...
  }, [formData, featuredImageUrl, selectedCategories, selectedTags])
  ...
  const restoreFromLocal = useCallback(() => {
    try {
      ...
      if (saved) {
        ...
        // 只有在有意义的数据时才恢复
        if (data.formData && (data.formData.title || data.formData.content)) {
          setFormData(data.formData)
          setFeaturedImageUrl(data.featuredImageUrl || '')  // 恢复图片URL
          setSelectedCategories(data.selectedCategories || [])
          setSelectedTags(data.selectedTags || [])
          setAutoSaveTime(new Date(data.timestamp))
          showAlert('success', '已恢复上次未保存的内容')
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (!user) {
        ...
        if (error) {
          ...
          showAlert('error', '认证失败，部分功能可能无法使用')
        } else {
  ...
  }, [formData, featuredImageUrl, selectedCategories, selectedTags, autoSaveToLocal])
  ...
  const loadCategoriesAndTags = useCallback(async () => {
    ...
    try {
      ...
      showAlert('error', '加载分类和标签失败，请刷新重试')
    } finally {
      setLoading(false)
    }
  }, [showAlert])
  ...
  useEffect(() => {
    ...
    initializeData()
  }, [loadCategoriesAndTags, restoreFromLocal, ensureAuthentication])

  // 页面离开事件处理器 - 单独的useEffect
  useEffect(() => {
  ...
  const handleInputChange = (field: string, value: string | boolean) => {
    setFormData(prev => {
      ...
      // 当标题变化时，自动生成SEO标题和slug
      if (field === 'title' && typeof value === 'string') {
        // 自动生成SEO标题
        if (value && !prev.meta_title) {
          newData.meta_title = value.slice(0, 60)
        }
        
        // 自动生成或更新slug（只有当slug为空或与当前标题生成的slug一致时）
        const currentGeneratedSlug = generateSlug(prev.title)
  ...
  const savePost = async (status: 'draft' | 'published') => {
    ...
    // 设置超时，确保状态能被重置
    const timeoutId = setTimeout(() => {
      console.error('保存操作超时，强制重置状态')
      setSaving(false)
      showAlert('error', '保存操作超时，请重试')
    }, 30000) // 30秒超时
    
    try {
      // 首先测试基本连接
      console.log('测试Supabase连接...')
      const { data: testData, error: testError } = await supabase.from('blog_categories').select('count').limit(1)
      if (testError) {
      ...
      console.log('创建文章，载荷:', postPayload)
      
      const post = await blogAPI.createPost(postPayload)
      ...
      showAlert('success', status === 'draft' ? '草稿保存成功！' : '文章发布成功！')
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (errorMsg.includes('bucket') || errorMsg.includes('storage')) {
          errorMessage = 'Supabase存储桶未配置或无权限访问，请检查存储配置'
        } else if (errorMsg.includes('policy') || errorMsg.includes('permission')) {
          errorMessage = '存储权限不足，请检查RLS策略配置'
        } else if (errorMsg.includes('network') || errorMsg.includes('fetch')) {
          errorMessage = '网络错误，请检查连接'
        } else if (errorMsg.includes('timeout')) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">新建文章</h1>
                <div className="flex items-center space-x-4 text-xs text-gray-500">
                  ...
                  {autoSaveTime && (
                    <span className="flex items-center">
                      ...
                      自动保存: {autoSaveTime.toLocaleTimeString()}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center space-x-3">
              {saving && (
                <Button 
                  variant="ghost" 
                  onClick={() => {
                    console.log('紧急重置保存状态')
                    setSaving(false)
                    showAlert('error', '已重置保存状态')
                  }}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      <span className={formData.title.length > 180 ? 'text-red-600' : formData.title.length > 160 ? 'text-yellow-600' : 'text-gray-500'}>
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-sm text-gray-500">
                      <span className={formData.excerpt.length > 270 ? 'text-red-600' : formData.excerpt.length > 240 ? 'text-yellow-600' : 'text-gray-500'}>
                        {formData.excerpt.length}/300 字符 (可选)
                      </span>
                    </p>
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="mt-1">
                    ...
                    />
                    <p className="mt-1 text-sm text-gray-500">
                      URL将自动根据标题生成，您也可以手动编辑。只能包含字母、数字和连字符。
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-sm text-gray-500">
                    <span className={formData.meta_title.length > 55 ? 'text-red-600' : formData.meta_title.length > 50 ? 'text-yellow-600' : 'text-gray-500'}>
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-sm text-gray-500">
                    <span className={formData.meta_description.length > 145 ? 'text-red-600' : formData.meta_description.length > 130 ? 'text-yellow-600' : 'text-gray-500'}>
